...
export default class LaunchedTest {
    ...
    render(){
        ...
        $('.launchTest').append(`<div class="card w-100"><div class="card-body p-4">${this.test[this.switch]}</div></div>`);
        //options
        this.load();
    ...
    finishtest(){
        ...
        $('.finish').on('click', async () =>{
            ...
            if ( response === 'late' ) $('.launchTest').append(`<div class="finish-block bg-primary">Время вышло<div>`);
            $('.launchTest').append(`<div class="finish-block bg-primary">Твой балл 
            ${response} из 100
            </div>`)
        } );
    ...
    switchTask(){
        ...
        if ( this.switch === 0 && this.switch + 1 !== this.test.length  ){
            $('.launchTest').append(`<button class="btn btn-primary next switch">Следующее задание</button>`);
        }
        else if ( this.switch + 1 === this.test.length && this.switch !== 0 )
        {
            if ( this.type !== 'article' ) $('.launchTest').append(`<button class="btn btn-primary previos switch">Предыдущее задание</button><button class="btn btn btn-success switch finish">Завершить тест</button>`);
            else $('.launchTest').append(`<button class="btn btn-primary previos switch">Предыдущее задание</button>`);
        }
        else if ( this.switch === 0 && this.switch + 1 === this.test.length )
        {
            if ( this.type !== 'article' ) $('.launchTest').append(`<button class="btn btn-success switch finish">Завершить тест</button>`);
        }
        else
        {
            $('.launchTest').append(`<button class="btn btn-primary previos switch">Предыдущее задание</button><button class="btn btn-primary next switch">Следующее задание</button>`);
        }
